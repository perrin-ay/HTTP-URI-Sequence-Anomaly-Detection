...
#all needs to happen in gpu here if present so use device var
...
def CNNparamaterStats(net,prntweights=False, VAE = False):
    ...
    # typically called before we send todevice so should be fine
    ...
    #        if 'ModuleList' in str(type(net.layers[i])):

    #            for l in net.layers[i]:
...
def FCNparameterStats(net,onlylinear=True,prntweights=False,verbose =False, VAE =  False):
 ...
 # typically called before we send todevice so should be fine
    ...
    if VAE:
        ...
        print ("###Weights and biases size in each Linear Layer of FCN###\n")
...
# best to change all of these to .detach in case this messes with flow of computation graph creation and freeing with backward 
...
# check here if detach can be run to grads    
...
#        concatallgrads = np.array([])
    ...
    #                        concatallgrads = torch.cat((concatallgrads, l.weight.grad.detach().flatten()))
    #                    except AttributeError:
    #                        pass
    #            else:
                try:    
...
def callback_prnt_weights_stats(net,modname):
    ...
    """
    # should be fine and dont need to move net back to cpu, since below operations can all happend on gpu
...
#        concatallweights = np.array([]) # if gpu you want them to be tensors and NOT NUMPY
    ...
    #                        concatallweights = torch.cat((concatallweights, l.weight.detach().flatten()))
    #                    except AttributeError:
    #                        pass
                try:
